-- Жёсткий fallback для старых клиентов Telegram:
-- если в INSERT board_tasks пришли несуществующие board_id/column_id,
-- создаём новую доску + 3 колонки и кладём задачу в первую колонку "Планы".
...
BEGIN
	...
	-- Fallback 1: колонка не найдена, но board_id валиден => выбираем первую колонку доски.
	...
	-- Владелец:
	-- 1) NEW.created_by, если это существующий auth.users.id;
	-- 2) последний встреченный owner из boards;
	-- 3) сервисный пользователь ingest (из миграции).
	IF NEW.created_by IS NOT NULL
		AND EXISTS (SELECT 1 FROM auth.users au WHERE au.id = NEW.created_by) THEN
